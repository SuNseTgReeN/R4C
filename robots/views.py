...
# Нужно переписатьНужно переписать код для отправки писем. Я хочу отправлять письма, при создании нового робота. К
# примеру: у нас есть Order в бд, в нём есть связь с customers, в нём в свою очередь email, куда отправлять письмо.
# Так же в Order есть серийный номер робота код для отправки писем. Я хочу отправлять письма, при создании нового
# робота. К примеру: у нас есть Order в бд, в нём есть связь с customers, в нём в свою очередь email, куда отправлять
# письмо. Так же в Order есть серийный номер робота